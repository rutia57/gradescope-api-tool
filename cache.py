...
def _hash_value(
    ...
    if isinstance(value, dict):
        return {k: _hash_value(v, hash_funcs) for k, v in sorted(value.items())}
    return value
    ...
    def decorator(func: Callable[P, R]) -> Callable[P, R]:
        ...
        def wrapper(*args: P.args, **kwargs: P.kwargs) -> R:
            ...
            key_bytes = pickle.dumps(
                (func.__module__, func.__qualname__, key_dict),
...
def send_email_with_attachment(attachment_path: str, subject: str, body: str, gmail_key_file: str) -> None:
...
def save_tracemalloc_file(snapshot: tracemalloc.Snapshot, filename: str="tracemalloc_memory_report.tsv", limit: int=200) -> str:
    ...
    with open(filepath, "w", newline="") as f:
        ...
        for stat in top_stats[:200]:
            writer.writerow([f"{stat.size / (1024*1024) :.2f} MB", stat.traceback])
    return filepath