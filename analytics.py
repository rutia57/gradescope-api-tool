...
def log_stats(firestore_key_file, firestore_collection_name): 
    ...
    result = {
        ...
        f'{button_name}_button_count': st.session_state.button_click_counts[st.session_state.state_hash][button_name]
        for button_name in [
    ...
    save_new_doc_to_firestore(stringify_keys(result), f'metadata_{str(st.session_state.session_id)}_{st.session_state.state_hash}', firestore_key_file, firestore_collection_name)
