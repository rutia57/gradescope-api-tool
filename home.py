...
with container:
    ...
    with error_logged_section(key_file, "Build authenticated Gradescope session"):
        if st.session_state.session_from_ext:
            try:
    ...
    with error_logged_section(key_file, "Set default session state values"):
    ...
    if st.session_state.gs_conn is not None:
        # Select course
        with error_logged_section(key_file, "Load course & assignment info"):
                ...
                with st.spinner('Loading assignment data... (for larger courses, this may take a couple of minutes)', show_time=True):
                    if st.session_state.selected_assignment_id == '<nan>': 
                        ...
                        try: 
                            ...
                            with error_logged_section(key_file, "Show assignment summary"):
                            ...
                            with error_logged_section(key_file, "Load course & assignment info"):
                                ...
                                with error_logged_section(key_file, "Show grade summary preview"):
                            ...
                            with error_logged_section(key_file, "Show grade feedback files section"):
                                ...
                                with st.expander('Select students and preview grade feedback', expanded=False):
                                    st.multiselect('Select students', users_with_grades, default=users_with_grades, format_func=lambda x: f'{x.first_name+" "+x.last_name:<{max_student_name_length+1}} [{x.email_address}]', key='selected_students_grades', 
                                                on_change=update_selected_student_grade_preview)
                                    if st.session_state.selected_students_grades: 
                            ...
                            with error_logged_section(key_file, "Show submission downloads section"):
                            ...
                            with error_logged_section(key_file, "Show assignment outline & stats section"):
                            ...
                            with error_logged_section(key_file, "Gradesd submissions export & download"):
    ...
    try:
        log_stats(firestore_key_file=key_file, firestore_collection_name=firestore_collection_name_key)
    except Exception:
